...
function checkLength(text) {
  ...
  return lines.length <= 2 && text.length <= 220;
}
...
function rateTone(text) {
  ...
  // Preachy markers
  if (/must|should always|you need to|important that you/i.test(text))
    score -= 1;
...
async function test1_RomanUrdu() {
  ...
  for (let i = 0; i < messages.length; i++) {
    ...
    console.log(
      `  Short (1-2 lines): ${checks.shortLength ? "PASS  ✅" : "FAIL ❌"}`,
    );
...
async function test2_PhaseTransition() {
  ...
  for (let i = 0; i < messages.length; i++) {
    ...
    const hasAdvice = adviceWords.some((w) =>
...
function test4_Validator() {
  ...
  const cases = [
    {
      input: "Theek hai, aap bata sakte ho?",
      expectValid: true,
      name: "Valid Roman Urdu",
    },
    { input: "آپ کیسے ہیں؟", expectValid: false, name: "Urdu Script" },
    {
      input: "That is completely fine and normal.",
      expectValid: false,
      name: "Full English",
    },
    {
      input: "Bohot mushkil lagta hai. Kya koi iske baare mein baat kari hai?",
      expectValid: true,
      name: "Valid mixed",
    },
  ];
  ...
  for (const tc of cases) {
    ...
    console.log(`\n  "${tc.input}"`);
    console.log(
      `  Expected valid: ${tc.expectValid} | Got: ${result.valid} | Issues: [${result.issues.join(", ")}]`,
    );
...
async function runAll() {
  ...
  console.log("═".repeat(60));
  console.log(
    `Test 1 (Roman Urdu): ${t1.filter((r) => r.allPass).length}/5 passed`,
  );
  console.log(
    `Test 2 (Phase Transition): ${t2.every((r) => r.phaseCorrect) ? "PASS ✅" : "FAIL ❌"}`,
  );
  console.log(`Test 3 (Tone Average): ${t3.average}/5`);
  console.log(
    `Test 4 (Validator): ${t4.filter((r) => r.pass).length}/4 passed`,
  );

  const pass1 = t1.every((r) => r.allPass);
  const pass2 = t2.every((r) => r.phaseCorrect && r.noEarlyAdvice);
  const pass3 = Number(t3.average) >= 4.0;
  const pass4 = t4.every((r) => r.pass);
