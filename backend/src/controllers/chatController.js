const { addMessage, incrementQuestion, getPhase, updatePhase, getHistory, getQuestionCount, resetState } = require('../services/conversationManager');
...
const chat = async (req, res) => {
    ...
    try {
        ...
        logger.info(`[PROMPT] Building prompt with history of ${history.length} messages...`);
        ...
        logger.info(`[CLEAN] Raw Response: "${rawBotResponse.substring(0, 50)}${rawBotResponse.length > 50 ? '...' : ''}"`);
        ...
        if (!validationResult.valid) {
            logger.warn(`[VALIDATOR] Issues found: ${JSON.stringify(validationResult.issues)}`);
        } else {
